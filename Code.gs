...
function sendDynamicNotification() {
  ...
  } else { // Defaults to email if blank or set to 'Email'
    showEmailDialog();
  }
}
 ...
 * This is a workaround for the script not being able to open a URL client-side from a trigger.
 */
...
function sendEmailFromDialog(row) {
  ...
  try {
    ...
    // Optionally, alert the user that it failed.
    SpreadsheetApp.getUi().alert("Failed to send email. Please check the logs for details.");
  }
}

/**
 * RESTORED: Generates and displays a WhatsApp "click to send" link for the currently selected row.
 */
...
function onFormSubmit(e) {
  // e.range gives us the range of the new row.
  const range = e.range;
  const sheet = range.getSheet();
  const row = range.getRow();

  // The submitted values are in the e.values array.
  // We access the raw medication string using its column index.
  const medicationsRaw = e.values[MEDS_COL - 1];

  // Check if the medication cell needs formatting.
  if (typeof medicationsRaw === 'string' && medicationsRaw.includes("~")) {
    let medListSheet = [];
    const meds = medicationsRaw.split("|");
    meds.forEach(med => {
      const details = med.split("~");
      // Format as "Medication Name - Dosage (Instructions)"
      medListSheet.push(`${details[0] || ''} - ${details[1] || ''} (${details[2] || ''})`);
    });
    // Overwrite the cell with the clean, multi-line list.
  ...
  // This column now simply tracks that this initial processing has been completed.
...
/**
 * Runs when a cell is edited. Only sends an automated email for the "Query" status.
 */
function onEdit(e) {
  const range = e.range;
  const sheet = range.getSheet();
  ...
  if (status === STATUS_QUERY) {
    ...
    try {
      ...
      Logger.log(`Error sending QUERY email for row ${row}: ${e.toString()}`);
      // We can't alert the UI here as it might be an automated edit. Logging is the best we can do.
    }
